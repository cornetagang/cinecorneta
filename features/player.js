...
if (grayscale) {
  art.on("ready", () => {
    ...
    art.controls.add({
      ...
      html: '<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18V4c4.42 0 8 3.58 8 8s-3.58 8-8 8z"/></svg>',
      click: () => {
        bynActivo = !bynActivo;
        const filtro = bynActivo ? "grayscale(100%) contrast(1.15) brightness(0.92)" : "";
        art.video.style.filter = filtro;
        art.notice.show = bynActivo ? "Modo Blanco y Negro activado 🎞️" : "Color restaurado";
      },
    });

    // Re-aplicar filtro al entrar/salir de pantalla completa
    art.on("fullscreen", () => {
      art.video.style.filter = bynActivo ? "grayscale(100%) contrast(1.15) brightness(0.92)" : "";
    });
